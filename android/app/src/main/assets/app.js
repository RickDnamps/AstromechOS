...
class AudioBoard {
  ...
  showUploadZone(show) {
    ...
    const reconcileRow = el('audio-reconcile-row');
    if (reconcileRow) reconcileRow.style.display = show ? 'block' : 'none';
  }

  // Reconcile the sounds index against the files actually present on the
  // Slave: drops ghost entries, files orphans into "others". Admin-only
  // (the row is gated by showUploadZone). Uses a 30s timeout because the
  // server does an SFTP listdir which can be slow on WiFi.
  async reconcileSounds(btn) {
    const run = async () => {
...
const cockpitPanel = {
  ...
  //   * "ALL HEALTHY"        — vibrant green when no live errors
  //   * "⚠ N ERRORS"         — bright red when any driver reports !ok
  //   * "⚠ COLLISION @ …"    — bright red, top priority (needs solder fix)
  //   * "⚠ DEGRADED — RESCAN"— amber when hats payload reports degraded
  // Whole row is clickable and routes to Settings → HATs.
  _hatHealthRow(data) {
    ...
    // LIVE driver errors — top priority, refreshed on every /status tick.
    ...
    // Collisions only surface from the hats payload (no other source).
    const collisions = [];
  ...
  _svcRow(label, cls, val, panel) {
    ...
    // Optional 4th arg `panel` makes the whole row a clickable shortcut
    // to a Settings sub-panel (uses the standard router so admin-gated
    // panels naturally land on the password prompt).
  ...
  _updateServices(data) {
    ...
    // Per the full-grid mapping (chantier 2026-05-30): every SERVICES row
    // is a clickable shortcut to its Settings sub-panel. _svcRow's 4th
    // arg adds the onclick + .cockpit-row-clickable class. The HAT health
    // single-line row sits exactly where the 3 legacy per-HAT rows used
    // to live, between BT Gamepad and Body Screen.
    svc.innerHTML =
  ...
  _updateActivity(data) {
    ...
    // Choreo / Audio activity rows route to their TOP-LEVEL tabs (not
    // a Settings sub-panel) so the operator lands on the player/editor
    // UI they actually need. ALIVE routes to Settings → Behavior where
    // the behavior_engine alive toggle lives.
...
class StatusPoller {
  ...
  async _pollOnce() {
    ...
    if (hatsPanel && hatsPanel.classList.contains('active')) {
      try {
        // Legacy meta calls (no-op now that inputs are removed; guarded
        // by el()?.value in validateHatField so they fail safely):
...
// ─────────────────────────────────────────────────────────────────
// Phase D 2026-05-28 — zero-config HAT status panel renderer.
// Consumes /hats/layout (LAN-open) + POST /hats/rescan (@require_admin).
// Uses XSS-safe DOM construction (createElement + textContent) — never
// innerHTML with user data.
// ─────────────────────────────────────────────────────────────────

function _hatRoleLabel(role) {
  const t = {
...
function _renderHatRow(host, hat) {
  // hat = {addr, role, chip, confidence, status?(derived), collision}
  const tr = document.createElement('tr');
  const tdHost = document.createElement('td'); tdHost.textContent = host;
  const tdAddr = document.createElement('td'); tdAddr.textContent = hat.addr || '—';
  const tdRole = document.createElement('td'); tdRole.textContent = _hatRoleLabel(hat.role);
  const tdChip = document.createElement('td'); tdChip.textContent = hat.chip || '—';

  // Derive a per-HAT status string for the badge:
  //   collision/chip='collision' → critical
  //   chip='pca9685'             → ready
  //   anything else              → degraded
  let perStatus = 'degraded';
  if (hat.collision || hat.chip === 'collision') perStatus = 'critical';
  else if (hat.chip === 'pca9685')               perStatus = 'ready';

  const tdStatus = document.createElement('td');
  const badge = document.createElement('span');
  badge.className = 'hat-status-badge hat-status-' + perStatus;
  badge.textContent = perStatus;
  tdStatus.appendChild(badge);

  const tdConf = document.createElement('td');
  tdConf.textContent = hat.confidence || '—';
...
async function renderHatsStatus() {
  const wrap   = el('hats-table-wrapper');
  const badge  = el('hats-aggregate-badge');
  const detail = el('hats-aggregate-detail');
  const banner = el('hats-critical-banner');
  const banAddrs = el('hats-critical-addrs');
  if (!wrap || !badge) return;

  let payload = null;
  try {
    payload = await api('/hats/layout');
  } catch (e) {
    badge.textContent = 'OFFLINE';
    badge.className = 'hat-status-badge hat-status-unknown';
    if (detail) detail.textContent = 'Could not reach /hats/layout';
    wrap.replaceChildren();
    if (banner) banner.style.display = 'none';
    return;
  }

  const agg = payload.aggregate_status || 'unknown';
  badge.textContent = agg;
  badge.className = 'hat-status-badge hat-status-' + agg;

  const masterPresent = payload.master?.present;
  const slavePresent  = payload.slave?.present;
  const masterCount   = (payload.master?.hats || []).length;
  const slaveCount    = (payload.slave?.hats  || []).length;
  if (detail) {
    const bits = [];
    bits.push(`Master: ${masterPresent ? masterCount + ' HAT' + (masterCount === 1 ? '' : 's') : 'no scan'}`);
    bits.push(`Slave: ${slavePresent ? slaveCount + ' HAT' + (slaveCount === 1 ? '' : 's') : 'no scan'}`);
  ...
  const thead = document.createElement('thead');
  const thr = document.createElement('tr');
  ['Host', 'Addr', 'Role', 'Chip', 'Status', 'Confidence'].forEach(t => {
    const th = document.createElement('th'); th.textContent = t; thr.appendChild(th);
  });
  thead.appendChild(thr); table.appendChild(thead);
  ...
  let hadAny = false;
  for (const host of ['master', 'slave']) {
    const side = payload[host] || {};
    if (!side.present) {
      const tr = document.createElement('tr');
      const td = document.createElement('td');
      td.colSpan = 6;
      td.textContent = host.toUpperCase() + ': no hw_layout.json yet — click RESCAN HARDWARE.';
      td.style.color = 'var(--text-dim, #888)';
      tr.appendChild(td);
    ...
    if (!(side.hats || []).length) {
      ...
      const td = document.createElement('td');
      td.colSpan = 6;
      td.textContent = host.toUpperCase() + ': scan ran, no HATs detected.';
      td.style.color = 'var(--amber, #ffaa44)';
      tr.appendChild(td);
      tbody.appendChild(tr);
      continue;
    }
    for (const hat of side.hats) {
      tbody.appendChild(_renderHatRow(host, hat));
      hadAny = true;
    }
  }
  table.appendChild(tbody);
  ...
  // CRITICAL banner + collision addr list
  const collisions = [];
  for (const host of ['master', 'slave']) {
    for (const h of payload[host]?.hats || []) {
      if (h.collision || h.chip === 'collision') {
        collisions.push(`${host} ${h.addr}`);
      }
    }
  }
  if (banner) {
    if (collisions.length) {
      banner.style.display = '';
      if (banAddrs) banAddrs.textContent = 'Conflicting HAT(s): ' + collisions.join(', ');
    } else {
      banner.style.display = 'none';
    }
  }
...
async function rescanHardware() {
  ...
  if (out) { out.textContent = 'Scanning Master + Slave...'; out.className = 'settings-status'; }
  if (btn) btn.disabled = true;
  try {
    const res = await apiDetail('/hats/rescan', 'POST', {});
    if (!res || !res.ok) {
      const msg = res?.error || 'Rescan failed';
      if (out) { out.textContent = '✗ ' + msg; out.className = 'settings-status error'; }
      toast(msg, 'error');
      return;
    }
    const d = res.data || res;  // apiDetail returns {ok,data} or raw
    const m = d.master || {};
    const s = d.slave  || {};
    const summary = `Master rc=${m.rc ?? '?'} · Slave rc=${s.rc ?? '?'}`;
    if (out) {
      const okBoth = m.rc === 0 && s.rc === 0;
      out.textContent = (okBoth ? '✓ ' : '⚠ ') + summary;
      out.className = 'settings-status ' + (okBoth ? 'ok' : 'error');
    }
    // Re-render straight away from the response (no extra round-trip)
    await renderHatsStatus();
    toast(`Rescan: ${summary}`, m.rc === 0 && s.rc === 0 ? 'ok' : 'info');
  } catch (e) {
    if (out) { out.textContent = '✗ ' + (e && e.message ? e.message : 'Rescan error'); out.className = 'settings-status error'; }
