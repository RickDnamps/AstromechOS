...
def create_app() -> Flask:
    ...
    from master.api.hats_bp         import hats_bp
    ...
    app.register_blueprint(hats_bp)
    ...
    # Boot reconcile: correct the sounds index against the Slave's real files
    # once at startup (catches drift from reboots / manual file changes).
    # Skips silently if the Slave isn't reachable yet — non-fatal.
    try:
        ...
        def _boot_reconcile():
            try:
                ...
                log.exception("Boot reconcile thread crashed")
