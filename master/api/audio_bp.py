...
# Alias for the local master index path. _sftp_delete_sound reads it to push
# the freshly-updated index to the Slave. Was referenced but never defined →
# every DELETE crashed with NameError in its background thread (the remote MP3
# was never removed). Defining it here fixes delete + the delete/reconcile race.
_SOUNDS_INDEX = _INDEX_FILE
...
def _list_slave_sound_stems():
    ...
    try:
        import paramiko
        # Serialize the listing on _sftp_lock so it can't observe a mid-delete
        # (or mid-upload) state — without it, a reconcile run during a delete's
        # SFTP window would re-discover the not-yet-removed file as an orphan.
        with _sftp_lock:
            c = paramiko.SSHClient()
            c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            c.connect(**_slave_sftp_creds())
        ...
        # paramiko.SSHException (auth / host-key / protocol) is NOT an OSError
        # subclass, so it must be named explicitly or it escapes to a 500.
...
def upload_sound():
    ...
    with _upload_lock:
        ...
        # Capacity check BEFORE transferring — don't ship a file we'll reject.
        # Audit finding A2-L3 2026-05-15: cap per-category sound count. 512
        # covers the biggest legit collection by a wide margin (R2-D2 ships
        # with 324 across all 14 cats). Caps stop a runaway script bloating
        # the index.
        _SOUNDS_PER_CAT_MAX = 512
        existing = cats.get(category, [])
        if final_stem not in existing and len(existing) >= _SOUNDS_PER_CAT_MAX:
            return jsonify({
                'ok': False,
                'error': f'Category "{category}" full (max {_SOUNDS_PER_CAT_MAX} sounds)',
            }), 400

        # Save to a local staging copy (source for the SFTP transfer). The
        # index is NOT mutated yet — only after the Slave confirms receipt.
        os.makedirs(_SOUNDS_DIR, exist_ok=True)
        f.save(dest_path)

    # Transfer to the Slave + verify, OUTSIDE _upload_lock (plays unaffected;
    # only other admin upload/delete ops serialize on the lock). TRANSACTIONAL:
    # the sound is indexed only once the Slave confirms it received the file.
    ...
    # Verified on the Slave. Commit to the index + push it.
    with _upload_lock:
        try:
            index = json.loads(Path(_INDEX_FILE).read_text(encoding='utf-8'))
    ...
    # Push the freshly-updated index + hot-reload the Slave's AudioDriver so
    # the new sound is immediately playable (no service restart).
    ...
    return jsonify({
        ...
        'filename': final_stem,
        'category': category,
        # Tell the UI when we auto-renamed so it can surface that fact to the
        # operator (e.g. "happy.mp3 → HAPPY_2"). Equal fields = no rename.
        'original': stem,
...
def _sftp_sync_index(index: dict) -> None:
    ...
    try:
        ...
        # paramiko.SSHException (auth/host-key/protocol failures) is NOT an
        # OSError subclass — it must be named explicitly. NoValidConnections
        # (slave down) and socket timeouts ARE OSError. Anything else
...
def _sftp_put_sound_verified(local_mp3: str, stem: str) -> bool:
    ...
    with _sftp_lock:
        try:
            ...
            # SSHException (auth/host-key/protocol) is NOT an OSError subclass;
            # naming it keeps a transfer failure on the transactional path
            # (return False → 502 + staging cleanup) instead of a 500.
