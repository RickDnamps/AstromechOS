...
if ip addr show wlan1 2>/dev/null | grep -q "inet "; then
    cd "$REPO"
    # Key off git's EXIT CODE, not stdout text: --ff-only exits non-zero on
    # every blocking case (dirty tree, non-ff, network) and zero on success
    # (including "Already up to date"). Immune to filenames in the summary
    # that happen to contain words like "error"/"fatal" (audit finding).
...
# Seed (*_default/, *.example) ships defaults in git; the app reads/writes
# the gitignored working dirs. rsync --ignore-existing / cp -n NEVER
# overwrite operator content — they only add new shipped files.
# ──────────────────────────────────────────────
...
seed_to_working "$REPO/slave/sounds_default" "$REPO/slave/sounds"
...
rsync -az --delete \
    ...
    --exclude='config/servo_angles.json' \
    --exclude='vendor/' \
...
# Populate the slave's working dirs from the seed (copy-new-only) — never
# overwrites the operator's sounds or slave.cfg (those are excluded from the
# rsync above; the seed sounds_default/ + slave.cfg.example DID sync).
