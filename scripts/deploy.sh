...
# Populate the slave's working dirs from the seed (copy-new-only). The app
# reads/writes the gitignored slave/sounds/ + slave.cfg; defaults ship in
# sounds_default/ + slave.cfg.example. --ignore-existing / cp never overwrite.
ssh $SSH_OPTS "${SLAVE_USER}@${SLAVE_HOST}" "mkdir -p ${SLAVE_REPO}/slave/sounds && rsync -a --ignore-existing ${SLAVE_REPO}/slave/sounds_default/ ${SLAVE_REPO}/slave/sounds/ && ([ -f ${SLAVE_REPO}/slave/config/slave.cfg ] || cp ${SLAVE_REPO}/slave/config/slave.cfg.example ${SLAVE_REPO}/slave/config/slave.cfg)" 2>/dev/null \
    && echo "      Slave working dirs ready" || echo "      WARN: slave seed->working failed"
